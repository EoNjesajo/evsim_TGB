...
class Room_scene(Entity):
    def __init__(self):
        ...
        self.CONNECTION_STRING ="HostName=wonshub.azure-devices.net;SharedAccessKeyName=serviceAndRegistryRead;SharedAccessKey=Hx96nI5ZXPKa4xRT80nAA5hpgmYzwxZzO7vwl8sMChw="
        self.DEVICE_ID = "maze"
    ...
    def login_window(self) :
        ...
        login_panel = WindowPanel(
            ...
            content=(
                Text('Chat_id:'),
                InputField(name='id_field', text = '5156920429'),
                Text('Name:'),
                InputField(name='name_field', text = 'won'),
...
class Infomation_Scene(Entity):
    ...
    def __init__(self):
        ...
        self.iothub_registry_manager = IoTHubRegistryManager(HUB_CONNECTION_STRING)

    def receive_message(self, message):
        map = list(vars(message)['data'])
        size = len(map)
    ...
    def iothub_messaging_sample_run(self, message):
        try:
            props={}
            props.update(messageId = str(5156920429))
            self.iothub_registry_manager.send_c2d_message(self.DEVICE_ID, message, properties=props)
    ...
    def maze_update(self,chat_id):
        ...
        query_result = self.iothub_registry_manager.query_iot_hub(query_spec, None, 100)
        for twin in query_result.items:
            data_dic  = eval(str(twin))
        map = data_dic['tags']['map']
        size = len(map)
        for i in range(0,size):
            for j in range(0,size):
                if map[i][j] == 1 :
